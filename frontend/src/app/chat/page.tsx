...
export default function ChatPage() {
  ...
  const load = async (myEmail: string, other: string) => {
    if (!other) return;
    ...
    setMessages((data || []).filter(m =>
      m.receiver_email === other || m.receiver_email === myEmail
    ));
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      <div className="max-w-xl mx-auto">
        ...
        <form onSubmit={start} className="mt-4 flex gap-2">
          <input required type="email" value={friendEmail} onChange={e => setFriendEmail(e.target.value)}
            placeholder="Friend's email" className="flex-1 px-4 py-3 rounded-2xl bg-white/10 outline-none" />
          <button className="px-4 py-3 rounded-2xl bg-white/10">Open</button>
        </form>
