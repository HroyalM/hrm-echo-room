...
export default function AppChrome({
  children, avatar, unread = 0,
}: { children: React.ReactNode; avatar?: string; unread?: number }) {
    ...
    <div className="min-h-screen bg-[#18191a] text-white pb-16">
      <header className="sticky top-0 z-30 bg-[#242526] border-b border-white/10 px-3 py-2 flex items-center justify-between">
        <div className="flex items-center gap-2">
          {path !== '/home' && <button onClick={() => router.back()} className="w-10 h-10 rounded-full bg-[#3a3b3c]">←</button>}
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href="/profile" className="w-10 h-10 rounded-full overflow-hidden bg-[#3a3b3c]">
            {avatar ? <img src={avatar} alt="me" className="w-full h-full object-cover" /> : <span className="flex h-full items-center justify-center">🙂</span>}
          </Link>
        </div>
      </header>
      {children}
